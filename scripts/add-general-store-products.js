/**
 * Script to add general store products with images
 * Downloads images and adds products to database via API
 */
...
async function addProducts() {
  ...
  for (let i = 0; i < generalStoreProducts.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Successfully added: ${product.title.en} (ID: ${data.product.id})\n`);
        successCount++;
      } else {
      ...
      console.error(`❌ Error adding product: ${product.title.en}`);
  ...
  console.log(`📦 Total: ${generalStoreProducts.length} products\n`);
}
...
module.exports = { generalStoreProducts, addProducts };
